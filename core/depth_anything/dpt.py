...
class DPTHead(nn.Module):
    ...
    def forward(self, out_features, patch_h, patch_w, return_intermediate=False, patch_size=14):
        ...
        with torch.cuda.amp.autocast(enabled=True):
            out = F.interpolate(out.float(), size=(int(patch_h * patch_size), int(patch_w * patch_size)),
...
class DPT_DINOv2(nn.Module):
    def __init__(self, encoder='vitl', features=256, out_channels=[256, 512, 1024, 1024], use_bn=False, use_clstoken=False, pretrained_dino=False):
        ...
        self.pretrained = torch.hub.load('/data/liangyingping_share/weiqizhe/.cache/torch/hub/facebookresearch_dinov2_main', f'dinov2_{encoder}14', source='local', pretrained=pretrained_dino)
