...
class AquaStereo(nn.Module):
    def __init__(self, args):
        ...
        self.conv1x1_layers = nn.ModuleList([
            ...
            nn.Conv2d(in_channels=96, out_channels=192, kernel_size=1, stride=1, padding=0, bias=False),
            nn.Conv2d(in_channels=192, out_channels=160, kernel_size=1, stride=1, padding=0, bias=False),
        ])
        ...
        self.down_convs = nn.ModuleList([
            nn.Conv2d(in_channels=c, out_channels=c, kernel_size=3, stride=2, padding=1)
            for c in [48, 64, 192, 160]  # 对应每一组特征通道数
        ])
    ...
    def forward(self, image1, image2, iters=12, test_mode=False):
        ...
        with amp.autocast(device_type='cuda', enabled=self.args.mixed_precision, dtype=getattr(torch, self.args.precision_dtype, torch.float16)):
            encoder=self.encoder(image1,image2)
            features_left_2x = [self.conv1x1_layers[i](f[0]) for i, f in enumerate(encoder[1:])]
            features_right_2x = [self.conv1x1_layers[i](f[1]) for i, f in enumerate(encoder[1:])]
            features_left = []
            features_right = []

            for i in range(4):
                f_left = features_left_2x[i]    # shape: [B, C, H, W]
                f_right = features_right_2x[i]  # shape: [B, C, H, W]

                f_left_down = self.down_convs[i](f_left)
                f_right_down = self.down_convs[i](f_right)

                features_left.append(f_left_down)
                features_right.append(f_right_down)
