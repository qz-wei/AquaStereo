...
class MY_FlowAugmentor:
    def __init__(self, crop_size, min_scale=-0.2, max_scale=0.5, do_flip=False, yjitter=False, saturation_range=[0.7,1.3], gamma=[1,1,1,1]):
        ...
        self.color_transform_1 = transforms.Compose([
            transforms.ColorJitter(0.4, 0.4, 0.2, 0.1),
            transforms.RandomGrayscale(p=0.2),
            transforms.GaussianBlur(kernel_size=9, sigma=(0.1, 2.0)),
            transforms.ToTensor(),
            transforms.Normalize(mean=(0.2587, 0.3512, 0.3446),
                                std=(0.2994, 0.3201, 0.3028))
        ])
    ...
    def spatial_transform(self, img1a, img2a, img1b, img2b, flow, valid):
        ht, wd = img1a.shape[:2]
        ...
        # Resize
        if (np.random.rand() < self.spatial_aug_prob) or (ht < self.crop_size[0]) or (wd < self.crop_size[1]):
            img1a = cv2.resize(img1a, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            img2a = cv2.resize(img2a, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            img1b = cv2.resize(img1b, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            img2b = cv2.resize(img2b, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
        ...
        if self.do_flip:
            ...
            if np.random.rand() < self.h_flip_prob and self.do_flip == 'h':
                img1a, img2a = img2a[:, ::-1], img1a[:, ::-1]
                img1b, img2b = img2b[:, ::-1], img1b[:, ::-1]
        ...
        # Crop with jitter margins
        margin_y = 20
        margin_x = 50
        y0 = np.random.randint(0, img1a.shape[0] - self.crop_size[0] + margin_y)
        x0 = np.random.randint(-margin_x, img1a.shape[1] - self.crop_size[1] + margin_x)

        y0 = np.clip(y0, 0, img1a.shape[0] - self.crop_size[0])
        x0 = np.clip(x0, 0, img1a.shape[1] - self.crop_size[1])

        img1a = img1a[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        img2a = img2a[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        img1b = img1b[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        img2b = img2b[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        flow = flow[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        valid = valid[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]

        return img1a, img2a, img1b, img2b, flow, valid


    def preprocess_student(self, img):
        img_pil = Image.fromarray(img)
        img_aug = self.geometric_transform(img_pil)
        tensor = self.color_transform_1(img_aug)
        return tensor
    
    def __call__(self, img1a, img2a, img1b, img2b, flow,valid):
        img1b, img2b = self.color_transform(img1b, img2b)
        img1b, img2b = self.eraser_transform(img1b, img2b)
        img1a, img2a = self.color_transform(img1a, img2a)
        img1a, img2a = self.eraser_transform(img1a, img2a)
        

        img1a, img2a, img1b, img2b, flow ,valid= self.spatial_transform(img1a, img2a, img1b, img2b, flow,valid)
        img1b = self.preprocess_student(img1b)
        img2b = self.preprocess_student(img2b)
        img1a = np.ascontiguousarray(img1a)
        img2a = np.ascontiguousarray(img2a)
        img1b = np.ascontiguousarray(img1b)
        img2b = np.ascontiguousarray(img2b)
        flow  = np.ascontiguousarray(flow)
        valid = np.ascontiguousarray(valid)
